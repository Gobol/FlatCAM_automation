...
class TclCommandAlignDrill(TclCommandSignaled):
    ...
    def execute(self, args, unnamed_args):
        ...
        if not ('dia' in args):
            return "ERROR: Specify -dia for hole diameter "

        tooldia = args['dia']

        if not ('dist' in args):
            return "ERROR: Specify -dist for distance from corners "
        ...
        def alligndrill_init_me(init_obj, app_obj):
            ...
            if 'holes' in args:
                ...
                localholes = (xmin-dist, ymin-dist), (xmax+dist, ymin-dist), (xmax+dist, ymax+dist), (xmin-dist, ymax+dist)
